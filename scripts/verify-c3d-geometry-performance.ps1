...
foreach ($density in $densities) {
    foreach ($style in $styles) {
        ...
        try {
            ...
            $gpuBufferReady = Get-ContractField $proxyLine 'gpuBufferReady'
            ...
            if ($renderCache -ne 'OpenGLVboIbo' -or
                $renderCacheReady -ne 'True' -or
                $gpuBufferReady -ne 'True') {
                throw "Static C3D GPU render cache was not ready: cache=$renderCache ready=$renderCacheReady gpu=$gpuBufferReady"
            }
