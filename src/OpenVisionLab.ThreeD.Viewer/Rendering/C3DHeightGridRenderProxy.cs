...
internal sealed class C3DHeightGridRenderProxy
{
    ...
    public const int MediumWireframeLineInterval = 2;
    public const int CoarseWireframeLineInterval = 4;
    public const int InteractionWireframeLineInterval = MediumWireframeLineInterval;
    ...
    private C3DHeightGridRenderProxy(
        ...
        int[] edgeIndices,
        int[] gridEdgeIndices,
        int[] interactionGridEdgeIndices,
        int[] coarseInteractionGridEdgeIndices,
        int[] surfaceEdgeIndices)
    {
        ...
        InteractionGridEdgeIndices = interactionGridEdgeIndices;
        CoarseInteractionGridEdgeIndices = coarseInteractionGridEdgeIndices;
    ...
    public int[] InteractionGridEdgeIndices { get; }

    public int[] CoarseInteractionGridEdgeIndices { get; }
    ...
    public int InteractionGridEdgeCount => InteractionGridEdgeIndices.Length / 2;

    public int CoarseInteractionGridEdgeCount => CoarseInteractionGridEdgeIndices.Length / 2;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var interactionGridEdgeIndices = CreateInteractionGridEdgeIndices(MediumWireframeLineInterval);
        var coarseInteractionGridEdgeIndices = CreateInteractionGridEdgeIndices(CoarseWireframeLineInterval);
        return new C3DHeightGridRenderProxy(
            points,
            triangleIndices.ToArray(),
            edgeIndices.ToArray(),
            gridEdgeIndices.ToArray(),
            interactionGridEdgeIndices,
            coarseInteractionGridEdgeIndices,
        ...
        int[] CreateInteractionGridEdgeIndices(int lineInterval)
        {
            var interactionWireframeStride = checked(pointStride * lineInterval);
            var result = new List<int>(gridEdgeIndices.Count / lineInterval);
            for (var index = 0; index < gridEdgeIndices.Count; index += 2)
            {
                var first = gridEdgeIndices[index];
                var second = gridEdgeIndices[index + 1];
                var firstPoint = points[first];
                var secondPoint = points[second];
                var horizontal = firstPoint.Row == secondPoint.Row;
                var coordinate = horizontal ? firstPoint.Row : firstPoint.Column;
                var minimum = horizontal ? minimumRow : minimumColumn;
                var maximum = horizontal ? maximumRow : maximumColumn;
                if (coordinate != minimum
                    && coordinate != maximum
                    && (coordinate - minimum) % interactionWireframeStride != 0)
                {
                    continue;
                }
