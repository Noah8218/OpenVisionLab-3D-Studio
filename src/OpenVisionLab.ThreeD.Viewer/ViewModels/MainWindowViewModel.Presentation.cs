...
public sealed partial class MainWindowViewModel
{
    ...
    {
        if (!double.IsFinite(zoomScale) || zoomScale <= 0.0)
        {
            throw new ArgumentOutOfRangeException(nameof(zoomScale));
        }

        if (ProjectionMode == ViewerProjectionMode.TopOrthographic)
        {
            OrthographicHeight = Math.Clamp(OrthographicHeight * zoomScale, 0.01, 100000.0);
            UpdateCameraStatus();
            return;
        }
    ...
    private void RefreshSceneContracts()
    {
        ...
        {
            layers.Add(new EntityLayer(
                inspectionSession.PreviewLayerId,
                inspectionSession.PreviewLayerName,
                LayerKind.Preview,
                ResultOverlayVisible,
                [inspectionSession.SourceEntityId]));
        }
    ...
    private void RefreshDisplaySettingsContext()
    {
        ...
        else
        {
            Display.ConfigureGeneratedGeometry(
                ResultOverlayVisible && inspectionSession.SourceEntityId == PointCloudEntityId);
        }
    ...
    {
        if (inspectionSession.ResultEntityId != LazTwoPointResultEntityId
    ...
    private ResultEntity CreatePublishedResultEntity(ToolResult result) =>
        new(
            inspectionSession.ResultEntityId,
            inspectionSession.ResultEntityName,
            inspectionSession.SourceEntityId,
            result.Status,
    ...
    private void ApplyActivePreviewResult()
    {
        ...
        {
            inspectionSession.Activate(ViewerInspectionKind.C3DWarpage);
            PreviewToolResult = c3dWarpagePreview;
        }
        else if (C3DSampleVisible && c3dThicknessPreviewActive && c3dThicknessPreview is not null)
        {
            inspectionSession.Activate(ViewerInspectionKind.C3DThickness);
        ...
        {
            inspectionSession.Activate(ViewerInspectionKind.C3DPointPairDimensions);
        ...
        {
            inspectionSession.Activate(ViewerInspectionKind.C3DHeightDeviation);
    ...
    {
        if ((inspectionSession.SourceEntityId != C3DEntityId && inspectionSession.SourceEntityId != C3DWarpageEntityId)
