...
{
    private double c3dFitDistance = 13.2;
    ...
    public void ApplyC3DCameraFit(
        Vector3 target,
        double distance,
        bool useTopInspectionView,
        string status)
    {
        if (!float.IsFinite(target.X)
            ...
            || distance <= 0.0)
        {
            throw new ArgumentOutOfRangeException(nameof(distance), "C3D camera fit must be finite and positive.");
        }
    ...
    public void ApplyTopOrthographicFit(
        Vector3 target,
        double height,
        ...
        string status)
    {
        if (!float.IsFinite(target.X)
            ...
            || height <= 0.0
            || !double.IsFinite(cameraDistance)
            || cameraDistance <= 0.0)
        {
            throw new ArgumentOutOfRangeException(nameof(height), "Top orthographic fit must be finite and positive.");
        }
    ...
    public void RestorePerspectiveView(string status = "Perspective view")
    {
        if (ProjectionMode == ViewerProjectionMode.Perspective)
        {
            return;
    ...
    public void ExitTopOrthographicForOrbit()
    {
    ...
    {
        inspectionSession.Activate(ViewerInspectionKind.LazTwoPointMeasurement);
