...
public static class ToolRecipeOrderedGraphExecution
{
    public static bool CanExecute(ToolRecipeDocument document, out string message)
    {
        ...
        var unsupported = document.Steps.FirstOrDefault(step => !ToolRecipeOrderedGraphStepExecutor.IsSupported(step.ToolId));
        ...
        try
        {
            ...
            for (var index = 0; index < rebound.Steps.Count; index++)
            {
                ...
                var execution = !step.OutputEnabled
                    ? new ToolRecipeOrderedGraphStepExecution(
                        new ToolResult(
                            step.ToolName,
                            ...
                            []),
                        null)
                    : ToolRecipeOrderedGraphStepExecutor.Execute(rebound, step, artifacts, cancellationToken);
                ...
                if (execution.Output is not null)
                {
                    ...
                    {
                        artifacts[ToolRecipeOrderedGraphStepExecutor.GetOutlierMaskArtifactId(step.OutputEntityId)] = execution.OutlierMask;
                    }
