...
internal static class ShellSmokeCommandLineOptionsVerification
{
    ...
    public static bool TryRun(string[] arguments, out bool passed, out string summary)
    {
        ...
        var checks = new List<(string Name, bool Passed)>
        {
            ...
            ("PreparationQualityComparison", VerifyPreparationQualityComparison()),
            ...
            ("IntegrationExchangeExe", VerifyIntegrationExchangeExe()),
    ...
    private static bool VerifyStepRemovalDialog()
    {
        ...
        [
            "shell.exe",
            "--smoke-step-removal-dialog",
            "--smoke-message-dialog-primary-pressed",
    ...
    private static bool VerifyPreparationQualityComparison()
    {
        var options = ShellSmokeCommandLineOptions.Parse(
            ["shell.exe", "--smoke-preparation-quality-compare"]);
        return options.PreparationQualityComparisonSmoke
            && options.FilterPreviewSmoke
            && options.ShouldAttachLoadedHandler(hasViewerSmokeScreenshot: false);
    }
    ...
    private static bool VerifySupportBundlePressed()
    {
        var options = ShellSmokeCommandLineOptions.Parse(
        [
            "shell.exe",
            "--smoke-support-bundle-pressed",
            "--shell-smoke-screenshot",
            "support-bundle-pressed.png"
        ]);
    ...
    private static bool VerifyIntegrationExchangeExe()
    {
        var options = ShellSmokeCommandLineOptions.Parse(
        [
            "shell.exe",
            "--smoke-integration-exe-role",
            "consumer",
            "--smoke-integration-exe-report",
            "integration-report.json"
        ]);
        return options.IntegrationExchangeExeRole == "consumer"
            && options.IntegrationExchangeExeReportPath == "integration-report.json"
            && options.ShouldAttachLoadedHandler(hasViewerSmokeScreenshot: false);
    }
}
