...
internal static class ToolThreePointPlaneWorkbenchVerification
{
    public static bool Verify(string reportPath, out string summary)
    {
        ...
        try
        {
            ...
            Check("open recipe for 3-Point Plane disposal", disposalWorkbench.TryOpenTeachingRecipe(recipePath, out var disposalOpen), disposalOpen);
            disposalWorkbench.SelectPipelineStep("step.plane.a");
            var disposalPreviewCompleted = disposalWorkbench.PreviewSelectedThreePointPlaneAsync().GetAwaiter().GetResult();
            disposalWorkbench.PublishSelectedStepCommand.Execute(null);
            var hadThreePointPlaneOutputBeforeDispose = disposalPreviewCompleted
                && disposalWorkbench.CurrentThreePointPlaneOutput is not null
                && disposalWorkbench.TryGetPublishedThreePointPlaneOutput("derived.plane.a", out _);
            disposalWorkbench.Dispose();
            disposalWorkbench.Dispose();
            Check(
                "3-Point Plane disposal clears Preview and published state idempotently",
                hadThreePointPlaneOutputBeforeDispose
                && !disposalWorkbench.IsThreePointPlanePreviewRunning
                && !disposalWorkbench.HasCurrentThreePointPlanePreview
                && !disposalWorkbench.IsThreePointPlanePreviewPublished
                && disposalWorkbench.CurrentThreePointPlaneOutput is null
                && !disposalWorkbench.TryGetPublishedThreePointPlaneOutput("derived.plane.a", out _)
                ...
                $"before={hadThreePointPlaneOutputBeforeDispose};running={disposalWorkbench.IsThreePointPlanePreviewRunning};current={disposalWorkbench.HasCurrentThreePointPlanePreview};published={disposalWorkbench.IsThreePointPlanePreviewPublished};output={disposalWorkbench.CurrentThreePointPlaneOutput is not null};registry={disposalWorkbench.TryGetPublishedThreePointPlaneOutput("derived.plane.a", out _)};preview={disposalWorkbench.PreviewSelectedStepCommand.CanExecute(null)};publish={disposalWorkbench.PublishSelectedStepCommand.CanExecute(null)};cancel={disposalWorkbench.CancelSelectedPreviewCommand.CanExecute(null)}");
        }
