...
internal static class ToolLineIntersectionWorkbenchVerification
{
    public static bool Verify(string reportPath, out string summary)
    {
        ...
        try
        {
            ...
            workbench.Dispose();
            Check(
                "Line Intersection disposal clears output and published registry",
                workbench.CurrentLineIntersectionOutput is null
                    && !workbench.TryGetPublishedLineIntersectionOutput("derived.corner.01", out _)
                    && !workbench.IsLineIntersectionPreviewRunning,
                "disposed Line Intersection owner is empty and fail-closed");
            var previewAfterDispose = workbench.PreviewSelectedLineIntersectionAsync()
                .GetAwaiter()
                .GetResult();
            workbench.Dispose();
            Check(
                "repeated Workbench disposal rejects new Line Intersection execution",
                !previewAfterDispose
