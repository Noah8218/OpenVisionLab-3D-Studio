...
internal static class ToolXyzAffineWorkbenchVerification
{
    ...
    public static bool Verify(string reportPath, out string summary)
    {
        ...
        try
        {
            ...
            var solveOutputBeforeDispose = workbench.CurrentAffineSolveOutput;
            workbench.Dispose();
            Check(
                "XYZ Affine disposal clears execution state and published registries",
                solveOutputBeforeDispose is not null
                    && workbench.CurrentAffineSolveOutput is null
                    && workbench.CurrentAffineApplyOutput is null
                    && !workbench.TryGetPublishedAffineSolveOutput(TransformId, out _)
                    && !workbench.TryGetPublishedAffineApplyOutput(CloudId, out _)
                    && !workbench.IsAffineSolvePreviewRunning
                    && !workbench.IsAffineApplyPreviewRunning,
                "disposed XYZ Affine owner is empty and fail-closed");
            var previewAfterDispose = workbench.PreviewSelectedXYZAffineSolveAsync()
                .GetAwaiter()
                .GetResult();
            workbench.Dispose();
            Check(
                "repeated Workbench disposal rejects new XYZ Affine execution",
                !previewAfterDispose
                    && !workbench.PreviewSelectedStepCommand.CanExecute(null),
