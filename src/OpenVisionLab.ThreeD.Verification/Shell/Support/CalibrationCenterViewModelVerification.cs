...
internal static class CalibrationCenterViewModelVerification
{
    ...
    public static bool Verify(string reportPath, out string summary)
    {
        ...
        try
        {
            ...
            var languageBeforeDisposalProbe = OpenVisionLanguageService.CurrentLanguage;
            var calibrationNotificationsBeforeDispose = propertyChanges.Count;
            shell.Dispose();
            shell.Dispose();
            var languageAfterDisposal = languageBeforeDisposalProbe == OpenVisionLanguage.English
                ? OpenVisionLanguage.Korean
                : OpenVisionLanguage.English;
            OpenVisionLanguageService.SetLanguage(languageAfterDisposal, save: false);
            Check(
                "Shell disposal releases Calibration localization subscription",
                calibrationNotificationsBeforeDispose == propertyChanges.Count,
                $"before={calibrationNotificationsBeforeDispose};after={propertyChanges.Count}");
            OpenVisionLanguageService.SetLanguage(languageBeforeDisposalProbe, save: false);

            using var standaloneCalibration = new CalibrationCenterViewModel();
            var standaloneNotifications = 0;
            standaloneCalibration.PropertyChanged += (_, _) => standaloneNotifications++;
            standaloneCalibration.Dispose();
            standaloneCalibration.Dispose();
            var standaloneProbeLanguage = OpenVisionLanguageService.CurrentLanguage == OpenVisionLanguage.English
                ? OpenVisionLanguage.Korean
                : OpenVisionLanguage.English;
            OpenVisionLanguageService.SetLanguage(standaloneProbeLanguage, save: false);
            Check(
                "Calibration disposal is idempotent and unsubscribes localization",
                standaloneNotifications == 0,
                $"notifications={standaloneNotifications}");
            standaloneCalibration.Dispose();
            OpenVisionLanguageService.SetLanguage(languageBeforeDisposalProbe, save: false);
