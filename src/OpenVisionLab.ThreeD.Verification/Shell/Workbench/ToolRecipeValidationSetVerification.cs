...
internal static class ToolRecipeValidationSetVerification
{
    public static bool Verify(string reportPath, out string summary)
    {
        ...
        try
        {
            ...
            Check(
                ...
                completenessWorkbench.ValidationThresholdCorrectionSummary);
            var thresholdFacadeNotifications =
                new HashSet<string>(StringComparer.Ordinal);
            completenessWorkbench.PropertyChanged += (_, args) =>
            {
                if (!string.IsNullOrWhiteSpace(args.PropertyName))
                {
                    thresholdFacadeNotifications.Add(args.PropertyName);
                }
            };
            var thresholdReplayCommandRefreshCount = 0;
            completenessWorkbench.ReplayValidationThresholdHeldOutCommand
                .CanExecuteChanged += (_, _) =>
                    thresholdReplayCommandRefreshCount++;
            completenessWorkbench.ReviewValidationThresholdCandidateCommand
                .Execute(null);
            ...
            Check(
                "threshold workflow owner forwards facade state and command refresh notifications",
                thresholdFacadeNotifications.IsSupersetOf(
                [
                    nameof(ToolWorkbenchViewModel
                    ...
                    nameof(ToolWorkbenchViewModel
                        .ValidationThresholdAssistantStage),
                    nameof(ToolWorkbenchViewModel
                        .ValidationThresholdAssistantSummary)
                ])
                && thresholdReplayCommandRefreshCount > 0,
                $"properties={string.Join(',', thresholdFacadeNotifications.OrderBy(name => name, StringComparer.Ordinal))};replayRefresh={thresholdReplayCommandRefreshCount}");
            Check(
            ...
            var disposalOwner = new ToolWorkbenchValidationSetExecutionOwner(() => { });
            using var disposalProgressEntered = new ManualResetEventSlim();
            using var releaseDisposalProgress = new ManualResetEventSlim();
            var disposalExecution = disposalOwner.ExecuteAsync(
                graphDocument,
                [
                    new ToolRecipeValidationSampleInput(
                        graphPassPath,
                ...
                new BlockingValidationProgress(
                    disposalProgressEntered,
                    releaseDisposalProgress));
            var disposalStarted = disposalProgressEntered.Wait(TimeSpan.FromSeconds(5));
            disposalOwner.Dispose();
            disposalOwner.Dispose();
            releaseDisposalProgress.Set();
            var disposalCanceled = false;
            try
            {
                disposalExecution.GetAwaiter().GetResult();
            }
            catch (OperationCanceledException)
            {
                disposalCanceled = true;
            }
            Check(
                "execution owner disposal cancels the active replay idempotently",
                disposalStarted
                && disposalCanceled
                && disposalOwner.IsDisposed
                && !disposalOwner.IsRunning
                && !disposalOwner.CanStart,
                $"started={disposalStarted};canceled={disposalCanceled};disposed={disposalOwner.IsDisposed};running={disposalOwner.IsRunning};canStart={disposalOwner.CanStart}");

            var gatedOwner = new ToolWorkbenchValidationSetExecutionOwner(() => { });
            using var validationProgressEntered = new ManualResetEventSlim();
            using var releaseValidationProgress = new ManualResetEventSlim();
            var blockingProgress = new BlockingValidationProgress(
                validationProgressEntered,
                releaseValidationProgress);
            var gatedExecution = gatedOwner.ExecuteAsync(
                graphDocument,
                [
                    new ToolRecipeValidationSampleInput(
                        graphPassPath,
                ...
                blockingProgress);
            var progressStarted = validationProgressEntered.Wait(TimeSpan.FromSeconds(5));
            var duplicateRejected = false;
            if (progressStarted)
            {
                var duplicateExecution = gatedOwner.ExecuteAsync(
                    graphDocument,
                    [
                        new ToolRecipeValidationSampleInput(
                            graphPassPath,
                ...
                try
                {
                    duplicateExecution.GetAwaiter().GetResult();
                }
            ...
            releaseValidationProgress.Set();
            gatedExecution.GetAwaiter().GetResult();
            Check(
                "validation execution owner rejects a concurrent replay without replacing cancellation state",
                progressStarted
                && duplicateRejected
            ...
            var hadValidationEvidenceBeforeDispose =
                workbench.HasValidationEvidence
                && workbench.ValidationThresholdCandidates.Count > 0;
            var hadThresholdWorkflowStateBeforeDispose =
                workbench.HasValidationThresholdAssistantAnalysis
                && workbench.HasValidationThresholdCandidates;
            workbench.Dispose();
            workbench.Dispose();
            navigationShell.Dispose();
            reopenedWorkbench.Dispose();
            Check(
                "workbench disposal releases Validation Set execution state idempotently",
                hadValidationEvidenceBeforeDispose
                && !workbench.IsValidationSetRunning
                && !workbench.CancelValidationSetCommand.CanExecute(null)
                && !workbench.RunValidationSetCommand.CanExecute(null),
                $"before={hadValidationEvidenceBeforeDispose};running={workbench.IsValidationSetRunning};cancel={workbench.CancelValidationSetCommand.CanExecute(null)};run={workbench.RunValidationSetCommand.CanExecute(null)}");
            Check(
                "workbench disposal releases Validation Threshold workflow state idempotently",
                hadThresholdWorkflowStateBeforeDispose
                && !workbench.HasValidationEvidence
                && !workbench.HasValidationThresholdCandidates
                && !workbench.HasValidationThresholdAssistantAnalysis
                && !workbench.HasValidationThresholdAssistantProposal
                && !workbench.HasValidationThresholdParameterChanges
                && !workbench.HasValidationThresholdHeldOutEvidence
                && !workbench.HasValidationThresholdDevelopmentEvidence
                && !workbench.ProposeValidationThresholdCandidateCommand.CanExecute(null)
                && !workbench.ReviewValidationThresholdCandidateCommand.CanExecute(null)
                && !workbench.CancelValidationThresholdReviewCommand.CanExecute(null)
                && !workbench.ApplyValidationThresholdCandidateCommand.CanExecute(null)
                && !workbench.RevalidateValidationThresholdCorrectionCommand.CanExecute(null)
                && !workbench.ReplayValidationThresholdHeldOutCommand.CanExecute(null),
                $"before={hadThresholdWorkflowStateBeforeDispose};evidence={workbench.HasValidationEvidence};candidates={workbench.HasValidationThresholdCandidates};analysis={workbench.HasValidationThresholdAssistantAnalysis};proposal={workbench.HasValidationThresholdAssistantProposal};parameterChanges={workbench.HasValidationThresholdParameterChanges};heldOut={workbench.HasValidationThresholdHeldOutEvidence};development={workbench.HasValidationThresholdDevelopmentEvidence}");
        }
    ...
    private sealed class BlockingValidationProgress(
        ManualResetEventSlim entered,
        ManualResetEventSlim release) : IProgress<ToolRecipeValidationProgress>
    {
        public void Report(ToolRecipeValidationProgress value)
        {
            if (value.CompletedStatus is null)
            {
                entered.Set();
                release.Wait();
            }
