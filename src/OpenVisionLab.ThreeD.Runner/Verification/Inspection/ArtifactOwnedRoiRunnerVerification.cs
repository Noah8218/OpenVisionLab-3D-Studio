        ...
        bool includeCrossSection = false)
    {
        ...
        if (includeMeasurement)
        {
            steps.Add(new ToolRecipeStep(
                ...
                ["derived.height-field", "selection.transformed.reference-roi", selections!.Single(item => item.Id == "selection.transformed.roi").Id], "result.thickness",
                [new("MinimumThickness", "0"), new("MaximumThickness", "0"), new("MinimumValidSampleCount", "4")]));
        }
        if (includeWarpage)
        {
            steps.Add(new ToolRecipeStep(
