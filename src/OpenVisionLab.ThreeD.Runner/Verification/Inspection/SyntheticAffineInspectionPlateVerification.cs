...
internal static class SyntheticAffineInspectionPlateVerification
{
    ...
    private static readonly ToolRecipeGridRectangle ThicknessReferenceRoi = new(35, 60, 10, 40);
        ...
        RunArtifactOptions? runArtifacts = null)
    {
        ...
        try
        {
            ...
            var thicknessExpected = CalculateThickness(field, ThicknessReferenceRoi, ThicknessRoi);
        ...
        C3DTransformedHeightField? field)
    {
        ...
        if (includeMeasurements)
        {
            ...
            var thicknessSelection = new ToolRecipeSelection(
                "selection.thickness-roi", "Known thickness measurement ROI",
            ...
            var thicknessReferenceSelection = new ToolRecipeSelection(
                "selection.thickness-reference-roi", "Known flat thickness reference ROI",
                ToolRecipeSelectionKinds.GridRectangle, SourceId, ReferenceFrame, transformedBinding,
                ThicknessReferenceRoi, null, null);
            ...
            selections.Add(thicknessReferenceSelection);
            selections.Add(thicknessSelection);
            selections.Add(warpageSelection);
            steps.Add(new ToolRecipeStep(
                "step.thickness", "thickness", "Thickness", 3,
                [HeightFieldOutputId, thicknessReferenceSelection.Id, thicknessSelection.Id], "result.thickness",
                [new("MinimumThickness", "14.4"), new("MaximumThickness", "15.6"), new("MinimumValidSampleCount", "800")]));
            steps.Add(new ToolRecipeStep(
    ...
    private static ThicknessTruth CalculateThickness(
        C3DTransformedHeightField field,
        ToolRecipeGridRectangle referenceRoi,
        ToolRecipeGridRectangle measurementRoi)
    {
        var referenceHeight = Values(field, referenceRoi).Average();
        var values = Values(field, measurementRoi).Select(value => value - referenceHeight).ToArray();
