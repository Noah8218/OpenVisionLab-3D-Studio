...
internal static class C3DThreePointPlaneGoldenVerification
{
    ...
    private static (bool Passed, string Evidence) VerifyRunnerReportAtomicity(string reportPath)
    {
        ...
        try
        {
            ...
            var recipePath = Path.Combine(directory, "three-point-plane.ov3d-teach.json");
            ToolRecipeDocumentStore.Save(recipePath, fixture.Document);
            var expected = ToolRecipeThreePointPlaneExecution.Execute(fixture.Document, fixture.StepId, directory);
            ...
            var firstExit = ToolRecipeThreePointPlaneRunnerExecution.Run(
                recipePath,
                fixture.StepId,
                runnerReportPath);
            ...
            var overwriteExit = ToolRecipeThreePointPlaneRunnerExecution.Run(
                recipePath,
                fixture.StepId,
                runnerReportPath);
            ...
            using (var lockStream = new FileStream(
                       ...
                       FileShare.None))
            {
                lockedExit = ToolRecipeThreePointPlaneRunnerExecution.Run(
                    recipePath,
                    fixture.StepId,
                    lockedPath);
            }
            ...
            var invalidParentExit = ToolRecipeThreePointPlaneRunnerExecution.Run(
                recipePath,
                fixture.StepId,
            ...
            var reportShape = firstText.Contains("ThreePointPlane|status=Pass", StringComparison.Ordinal)
                && firstText.Contains(expected.Output.ContentSha256, StringComparison.Ordinal)
                && overwriteText.Contains("ThreePointPlane|status=Pass", StringComparison.Ordinal)
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        {
            var root = rootOverride ?? Path.Combine(Path.GetTempPath(), "OpenVisionLab.ThreeD", "ThreePointPlane", Guid.NewGuid().ToString("N"));
