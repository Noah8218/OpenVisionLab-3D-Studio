...
internal static class C3DPointCloudBackgroundFilterGoldenVerification
{
    ...
    private static (bool Passed, string Evidence) VerifyRunnerReportAtomicity(string fixtureDirectory)
    {
        ...
        try
        {
            var current = CreateCurrent("current.point-cloud.atomic");
            var background = CreateBackground("background.point-cloud.atomic");
            var direct = Evaluate(
                current,
                background,
                "filtered.point-cloud.atomic",
                "step.point-cloud.atomic");
            if (!IsPass(direct) || direct.Output is null || direct.Evidence is null)
            {
                return (false, $"direct={direct.Result.Status}:{direct.Result.Message}");
            }

            var sourceSpecificationPath = Path.Combine(directory, "point-cloud-background-filter.json");
            var firstOutputPath = Path.Combine(directory, "filtered-point-cloud-first.json");
            var secondOutputPath = Path.Combine(directory, "filtered-point-cloud-second.json");
            ...
            var specification = CreateSpecification(
                ...
                firstOutputPath);
            specification.StepId = "step.point-cloud.atomic";
            File.WriteAllText(
                sourceSpecificationPath,
                JsonSerializer.Serialize(specification, new JsonSerializerOptions { WriteIndented = true }),
                new UTF8Encoding(encoderShouldEmitUTF8Identifier: false));

            var firstExit = C3DPointCloudBackgroundFilterRunnerExecution.Run(
                sourceSpecificationPath,
                reportOutputPath);
            ...
            var overwriteExit = C3DPointCloudBackgroundFilterRunnerExecution.Run(
                sourceSpecificationPath,
                reportOutputPath);
            ...
            var secondOutputBytes = File.Exists(secondOutputPath)
            ...
            using (var lockStream = new FileStream(
                       ...
                       FileShare.None))
            {
                specification.OutputPath = Path.Combine(directory, "filtered-point-cloud-locked.json");
                File.WriteAllText(
                    sourceSpecificationPath,
                    JsonSerializer.Serialize(specification, new JsonSerializerOptions { WriteIndented = true }),
                    new UTF8Encoding(encoderShouldEmitUTF8Identifier: false));
                lockedExit = C3DPointCloudBackgroundFilterRunnerExecution.Run(
                    sourceSpecificationPath,
                    lockedPath);
            }
            ...
            specification.OutputPath = Path.Combine(directory, "filtered-point-cloud-invalid-parent.json");
            File.WriteAllText(
                sourceSpecificationPath,
                JsonSerializer.Serialize(specification, new JsonSerializerOptions { WriteIndented = true }),
                new UTF8Encoding(encoderShouldEmitUTF8Identifier: false));
            var invalidParentExit = C3DPointCloudBackgroundFilterRunnerExecution.Run(
                sourceSpecificationPath,
            ...
            var outputNoBom = !HasUtf8Bom(firstOutputBytes)
                && !HasUtf8Bom(secondOutputBytes);
