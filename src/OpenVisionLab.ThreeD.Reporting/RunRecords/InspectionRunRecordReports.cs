...
/// <summary>
/// Writes HTML and CSV from completed Run Record evidence, without reading sources or rerunning inspection.
/// </summary>
public static class InspectionRunRecordReports
{
    // Keep report formatting here. Execution identity/environment and output selection stay with callers.
    // These formats deliberately preserve legacy columns, escaping, encoding and unavailable evidence text.
    // Extend this owner in place; another split requires a concrete contract or dependency problem.

    public static void WriteHtml(string path, InspectionRunRecord record)
    {
    ...
    public static void WriteCsv(string path, InspectionRunRecord record)
    {
        EnsureDirectory(path);
