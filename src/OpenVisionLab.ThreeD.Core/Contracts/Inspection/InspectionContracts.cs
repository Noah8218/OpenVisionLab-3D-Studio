    ...
    double Scale)
{
    ...
    {
        var position = sourcePosition * (float)Scale;
        position = Vector3.Transform(position, Matrix4x4.CreateRotationX(ToRadians(RotateXDegrees)));
        position = Vector3.Transform(position, Matrix4x4.CreateRotationY(ToRadians(RotateYDegrees)));
        position = Vector3.Transform(position, Matrix4x4.CreateRotationZ(ToRadians(RotateZDegrees)));
        return position + new Vector3((float)TranslateX, (float)TranslateY, (float)TranslateZ);
    }
