...
internal static class ToolWorkbenchToolCatalog
{
    ...
    [
        ...
        new("Prepare", "Level Surface", "level-surface", 2, "Raw C3D HeightField + one or more GridRectangles", "LeveledHeightField + LevelingTransform", "Fit one plane across explicit reference ROIs and remove its X/Z raw-height trend without changing the source grid or missing mask.", [new("ReferenceFitPolicy", "LeastSquaresHeightPlane"), new("LevelingPolicy", "HeightDetrendToReferenceMean"), new("MissingValuePolicy", "PreserveMask"), new("GridPolicy", "PreserveSourceGrid"), new("MinimumValidSampleCount", "16"), new("MaximumReferenceRmsResidual", "5")]),
        ...
        new("Measure", "Completeness Grid", "completeness-grid", 3, "HeightField + Reference GridRectangle + Inspection GridRectangle", "CompletenessGridMetrics", "Generate deterministic native-grid cells, apply inclusive finite-coverage and reference-relative height limits, then publish cell and aggregate Pass/Fail evidence.", [new("Rows", "2"), new("Columns", "2"), new("XPitchColumns", "2"), new("ZPitchRows", "2"), new("CellWidthColumns", "2"), new("CellHeightRows", "2"), new("CellShape", "GridRectangle"), new("MinimumFiniteCoverageRatio", "0.95"), new("MinimumReferenceRelativeMeanRawHeight", "-100000"), new("MaximumReferenceRelativeMeanRawHeight", "100000")]),
        new("Measure", "Presence Check", "presence-check", 2, "HeightField + GridRectangle", "PresenceCheckResult", "Evaluate one explicitly authored feature using inclusive finite-coverage and raw-height limits; missing finite mean fails closed.", [new("MinimumFiniteCoverageRatio", "0.95"), new("MinimumMeanRawHeight", "-100000"), new("MaximumMeanRawHeight", "100000")]),
