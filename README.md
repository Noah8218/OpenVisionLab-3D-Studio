...
- Current rule scope: C3D height deviation plus complete typed plane-flatness, explicit C3D point-pair distance/width/signed-angle, two-region signed Gap/Flush, reference-plane Volume, exact-row Cross-section Dimensions, and feature-first C3D Thickness ROI Teaching. Thickness supports C3D load, one taught grid ROI, editable limits, explicit Preview/Publish, recipe save/reopen, headless Runner replay, and shared Viewer/Shell result surfaces. The local C3D scalar remains `raw-height`, not calibrated physical thickness.
...
```powershell
dotnet run --project src\OpenVisionLab.ThreeDStudio\OpenVisionLab.ThreeDStudio.csproj -c Debug --no-build -- --smoke-screenshot artifacts\c3d_grayscale.png --smoke-contracts artifacts\c3d_grayscale.txt --smoke-c3d thickness --smoke-action color-grayscale
dotnet run --project src\OpenVisionLab.ThreeDStudio\OpenVisionLab.ThreeDStudio.csproj -c Debug --no-build -- --smoke-screenshot artifacts\c3d_thermal.png --smoke-contracts artifacts\c3d_thermal.txt --smoke-c3d thickness --smoke-action color-thermal
```

The maps are display-only normalized-height views. They do not change measurement or add calibrated physical-height ranges. See `docs\OPENVISIONLAB_3D_C3D_COLOR_MAPS_20260715.md`.

C3D Thickness Teaching golden and recipe replay:
...
This is a typed C3D grid-ROI inspection slice with explicit Preview/Publish and Runner replay. Its `raw-height` values are not calibrated physical thickness. See `docs\OPENVISIONLAB_3D_C3D_THICKNESS_TEACHING_20260717.md`.
