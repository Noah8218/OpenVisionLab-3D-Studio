...
5. Load `Tetrahedron.stl` to verify a non-GLB triangle mesh path with no texture or vertex-color metadata.
6. Load `xyzrgb_manuscript.laz` after the point-cloud loader path exists. Verify unit metadata, coordinate ranges, point count, RGB availability, and downsample behavior before rendering all points.
7. Load `interesting.las` to verify uncompressed LAS RGB decoding on a small 1,065-point sample.
