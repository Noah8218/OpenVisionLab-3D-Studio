...
4. Load `Avocado.glb` to verify a non-box textured mesh with realistic framing and material metadata.
5. Probe `ToyCar.glb` when checking larger GLB behavior without adding it to the fixed matrix; it is useful for stress-checking texture, bounds, pick, and two-point measurement on a high-triangle real model.
6. Load `Tetrahedron.stl` to verify a non-GLB triangle mesh path with no texture or vertex-color metadata.
7. Probe `3DBenchy.stl` only when checking large STL behavior. It passes Viewer/Shell STL load, pick, and two-point measurement with imported-mesh render sampling active; keep it probe-only until the team decides the extra routine smoke time is worth the fixed-matrix coverage.
