...
| Runner and evidence parity | 2 | Headless replay, contract comparison, screenshots, result layers, Shell history/snapshot views, and schema `1.2` JSON/HTML/CSV with typed step/source/reference identity. Two authenticated schema `1.1` Windows Cross-section runs reproduce the same normalized payload. | Repeatability covers the same source/recipe only; multi-piece, batch, and a general ordered multi-step replay contract remain unproven. |
