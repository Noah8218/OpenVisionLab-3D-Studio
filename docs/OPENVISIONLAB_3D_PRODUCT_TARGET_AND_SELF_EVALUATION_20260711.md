...
- Fixed NIST nominal/actual end-to-end evidence: `artifacts/nominal_actual_publish_20260714` records accepted Viewer and Shell captures, typed recipe reopen parity, `ViewerRunnerComparison|Matched`, and schema `1.2` JSON plus HTML/CSV. `artifacts/nominal_actual_render_density_20260714` records before/after Fast/Balanced/Detailed captures and one normalized measurement SHA-256. `artifacts/nominal_actual_selected_point_20260714` records the current `27/27` executor/result and `65/65` ViewModel checks, deterministic selected-point provenance, `128/128` fixed-matrix regression, all typed goldens, and BinaryHost preservation. `artifacts/pointer_input_regression_20260715` records real WPF pointer input, byte-identical repeat reports, accepted Viewer/Shell before/after captures, the current `128/128` matrix, and BinaryHost preservation.
...
| Runner and evidence parity | 2 | Headless replay, contract comparison, screenshots, result layers, Shell history/snapshot views, schema `1.2` JSON/HTML/CSV, fixed NIST actual/nominal/query identity, and `Matched` output. | Repeatability is still same-source/fixed-pair evidence; multi-piece, second-pair, batch, and a general ordered multi-step replay contract remain unproven. |
| Nominal/actual comparison | 2 | Fixed NIST actual/nominal/query identities, source-provided frame/alignment, exact point-to-triangle deviation, robust signs, zero-centred color map, tolerances, Preview/Publish, recipe roundtrip, and Runner parity. | No second pair, non-identity alignment proof, arbitrary sampling/topology validation, uncertainty budget, or metrology certification. |
...
End-to-end status on 2026-07-14: the NIST Overhang X4 pair has a verified CloudCompare full-vertex signed/unsigned C2M reference in its documented 3-2-1 part frame. The original measured STL stream, controlled signed-distance golden, fixed-frame algorithm parity, View, ViewModel, shared execution, real signed-color Preview, separate Publish result, typed recipe save/reopen, Runner replay, schema `1.2` Run Record, and current Viewer/Shell evidence gates pass for this pair. The next gate requires a second genuinely distinct pair and independently known non-identity transform truth; do not generalize from this one identity-frame case.
