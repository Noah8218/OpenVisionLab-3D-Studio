...
> one coordinate-true view-only cursor with the main 3D Viewer. Continue with
> `C-09/C-10` synchronized ROI display/editing. The visible invalid-cell
> overlay remains `C-11`; physical calibration and metrology remain
> unverified. See
> `OPENVISIONLAB_3D_MASTER_DEVELOPMENT_WORKFLOW_AND_BACKLOG_20260727.md` and
> `OPENVISIONLAB_3D_SHARED_HEIGHT_CURSOR_20260728.md`.

> 2026-07-27 Height Image update: `C-06` is complete. The Workbench now
> provides a full-size coordinate-true Height Image in side-by-side, stacked,
> and reusable pop-out auxiliary hosts. One C3D cell maps to one native pixel
> with `X=column`, `Y=row`, no flip, and no resampling. Fit, 1:1, zoom, pan,
> and raw-height hover remain view-only. The exact Synthetic Thickness Coupon v1 Thickness source
> produces `1,075,200` pixels and current mapping verification passes `11/11`.
> This does not yet provide explicit invalid-mask parity, shared 2D/3D hover,
> Height Image ROI editing, manual numeric display range, calibration, or
> metrology. Continue with `B-09`, then `B-08`. See
> `OPENVISIONLAB_3D_FULL_HEIGHT_IMAGE_VIEWER_20260727.md`.

> 2026-07-27 Source Trust update: `B-07 SourceQualityReport` schema `1.0`
> is complete as a WPF-neutral Core contract with deterministic Data analysis
> and Runner JSON output. Focused verification passes `12/12`; the exact
> Synthetic Thickness Coupon v1 Thickness source reports `1280 x 840`, `908,436` valid, and
> `166,764` missing cells. Unsupported channels remain explicitly
> unavailable rather than inferred. `C-06` Height Image is now complete, but
> the visible invalid-cell map, Source Quality workspace, physical
> calibration, and metrology remain open. Continue with `B-09` and `B-08`.
> See `OPENVISIONLAB_3D_SOURCE_QUALITY_REPORT_20260727.md`.

> 2026-07-27 master development workflow update: commercial-video-derived
> product work is now decomposed into release trains R0-R6 and 234 unique
> backlog items: 67 Complete, 19 Partial, 123 New, 9 External prerequisite,
> and 16 Out of scope. The master records dependencies, closure evidence, the
> per-slice implementation loop, Definition of Done, and the executable
> queue. R0 remains the owner's unaided current Release replay.
> SourceQualityReport and the linked Height Image foundation are complete;
> continue with mask parity and Source Quality UI rather than Surface
> Matching or hardware integration. See
> `OPENVISIONLAB_3D_MASTER_DEVELOPMENT_WORKFLOW_AND_BACKLOG_20260727.md`.

> 2026-07-27 commercial-video direction update: all 11 owner-supplied GoPxL,
> SICK Nova, HALCON/MERLIC, Zivid Studio, and Photoneo videos under
...
> inspection workbench. After the owner's unaided Inspection Workspace v3
> replay, current priority is Source Quality, a full-size linked Height Image
> workspace, a separate typed `OrientedBox3D`, evidence-based threshold
> teaching, completeness/cell inspection, typed preparation tools, and only
> then surface matching. Camera acquisition, stereo reconstruction,
> PLC/robot/fieldbus/HMI, cloud, and plant management remain out of scope.
> This newer source controls commercial-video-derived priorities when older
> market notes conflict. See
> `OPENVISIONLAB_3D_COMMERCIAL_VIDEO_DIRECTION_AND_PRIORITY_20260727.md`.

> 2026-07-24 Release performance update: the input-first actual Shell EXE now reuses the exact loaded Viewer C3D identity for Workbench source binding, and already-empty teaching/tool overlay clears no longer cause redundant synchronous renders. On the largest available `1621 x 3317`, `21.5 MB` source, Release whole transition fell `1,563 -> 565 ms` and Workbench binding/state fell `886.538 -> 32.951 ms`. A seven-source, six-grid, three-repetition Release actual-EXE matrix passes `21/21` with VBO/IBO, GPU-ready state, zero fallback, and bounded apply/Workbench stages. This is local Windows/NVIDIA GTX 1060 evidence only; it does not change the owner-approved UI `85/100` score, prove arbitrary-large-data or multi-GPU portability, or establish physical/metrology trust. See `OPENVISIONLAB_3D_SYNTHETIC_THICKNESS_SAMPLE_MIGRATION_20260728.md`.

> 2026-07-23 result-access update: the existing docked bilingual Run Record tab now exposes compatible JSON open, current JSON/HTML/CSV and folder access, collision-safe bundle export, and ten recent valid paths. Open/reopen is read-only, invalid JSON retains the current record, and exported artifacts are byte-identical. Focused verification passes `8/8`, docking `27/27`, teaching `25/25`, and the controls remain usable at `1920 x 1040` and `1280 x 760`. This improves local result retrieval but does not change the accepted UI `85/100` score without a new owner review, and it is not a production database, batch/trend platform, retention policy, calibration, or metrology evidence. See `OPENVISIONLAB_3D_RUN_RECORD_HISTORY_UX_20260723.md`.

> 2026-07-23 durable evidence update: the production Runner now maps the one existing general sequential typed executor into schema `1.4` JSON/HTML/CSV and the existing bilingual docked Run Record view. The fixed 27-step Synthetic Affine Plate passes `27/27`; a controlled same-grid measurement sample completes expected Fail `27/27` while preserving later evidence. Each step retains exact authored order, typed route, status/message/time, metrics, overlays, and output-content SHA-256, including feature-only HTML/CSV rows. Schema `1.3` and schema `1.2` paths remain compatible. This closes durable reporting for registered sequential tools, not arbitrary DAG execution, production batch/history infrastructure, trusted real alignment, physical calibration, or metrology. See `OPENVISIONLAB_3D_GENERAL_GRAPH_RUN_RECORD_20260723.md`.

> 2026-07-23 execution update: the dockable bilingual Validation Set now replays each selected same-grid C3D sample through the complete currently supported typed chain in authored INPUT -> OUTPUT order, from Filter and feature/datum tools through full-XYZ affine A1/A2/A3 and all seven measurement adapters. Per-sample raw and A3 selection rebinding is ephemeral, upstream errors fail closed, measurement Fail preserves later evidence, and the authored recipe/Viewer input remain unchanged. The fixed 27-step synthetic recipe completes Pass; a modified measurement sample completes Fail with later evidence; a missing-edge sample stops at step 2; selected output hashes match the established direct adapters. Current evidence passes build `0/0`, Validation Set `16/16`, and Synthetic Affine Plate `18/18`. This closes general sequential software replay for registered adapters, not arbitrary DAG execution, production batch infrastructure, physical calibration, or metrology. See `OPENVISIONLAB_3D_ORDERED_GRAPH_VALIDATION_20260723.md`.

> 2026-07-22 architecture update: the canonical UI and recipe lifecycle is the generic `Inspection Recipe` Workbench using `ToolRecipeDocument`. Thickness, Warpage, Plane Flatness, Point Pair Dimensions, Gap/Flush, Volume, and Cross-section Dimensions are Measure tool steps, not product modes. Schema `1.3` preserves exact `TransformedHeightField` GridRectangle/PointSet ownership, and Generic Ordered Recipe Executor v1 replays A3 then every following supported measurement step in authored order with direct-adapter hash parity. Cross-section Dimensions consumes one exact A3 row segment and independently judges U-axis width and H-axis range; it is not automatic edge detection or calibrated physical dimensioning. The bounded executor now emits one schema `1.3` JSON/HTML/CSV Run Record containing A3 plus all seven ordered measurement steps, and the current bilingual docked Workbench displays the same typed routes and states; focused verification passes `21/21`. Existing single-step schema `1.2` remains compatible. Reusable Plane Fit/Flatness, Point Pair, Gap/Flush, Volume, and Cross-section arithmetic is owned by committed Library-Noah `Lib.ThreeD 2.7.9`, while Studio retains identity/UI/recipe/result adaptation. See `OPENVISIONLAB_3D_MULTI_STEP_RUN_RECORD_20260722.md` plus the generic Measure documents. The historical owner-supplied Thickness/Warpage videos contribute only general synchronized step/overlay/properties/results and repeated-instance lessons; their visual identity, fixed layout, and fixed algorithm taxonomy are excluded. `Synthetic Affine Inspection Plate v1` remains the deterministic whole-chain `16/16` software golden. Automatic feature detection, real trusted four-landmark A1/A2 replay, and physical/metrology trust remain blocked by missing trusted evidence.
...
- Full-resolution external runtime evidence: CloudCompare 2.13.2 preserved all 1,653,562 ordered vertices and RGB at `1e-6`, reported sub-micro-unit C2C statistics, and preserved the fixed recipe point-pair display-frame metrics. Evidence is recorded under `artifacts/map_fidelity_cloudcompare_20260713` and summarized in `docs/OPENVISIONLAB_3D_SYNTHETIC_THICKNESS_SAMPLE_MIGRATION_20260728.md`.
...
Multi-step status on 2026-07-22: schema `1.3` adds optional ordered `Steps` while retaining schema `1.2` single-step output. The bounded explicit-A2 executor records A3 Re-grid followed by Thickness, Warpage, Plane Flatness, Point Pair, Gap/Flush, Volume, and Cross-section in one JSON/HTML/CSV result with exact typed routing and per-step evidence. A deliberate Thickness tolerance Fail determines the aggregate Fail but later Pass evidence remains present. The bilingual current Workbench displays the same eight-step record in its docked read-only Run Record tab. Focused verification passes `21/21`; this is not arbitrary graph execution, batch/multi-piece infrastructure, or metrology evidence.

General graph status on 2026-07-23: schema `1.4` retains ordered `Steps` and adds optional per-step `OutputContentSha256`. The production Runner records the complete currently registered 27-step sequential typed graph through the same execution owner used by Validation Set. JSON, HTML, CSV, and the bilingual docked Run Record view retain feature, transform, and measurement rows with exact routes and evidence. The fixed source passes `27/27`; a controlled tolerance-failure source completes expected Fail `27/27` and retains later Warpage evidence. Writer verification passes `21/21`, and the schema `1.3` bounded record remains green at `21/21`. This closes durable general sequential reporting, not arbitrary DAGs, production batch/history, real multi-piece alignment, or metrology.
