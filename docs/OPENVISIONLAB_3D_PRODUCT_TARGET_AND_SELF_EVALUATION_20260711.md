...
| Nominal/actual comparison | 2 | Two fixed NIST identities, source-provided frame/alignment, exact point-to-triangle deviation, robust signs, zero-centred color map, tolerances, Preview/Publish, recipe roundtrip, Runner parity, supplied-transform evidence, local difficult-geometry goldens, and a mandatory fail-closed workflow definition. | Registration recovery, arbitrary sensors/geometry, uncertainty, and metrology remain open; the new workflow gate still needs its first GitHub Windows run. |
