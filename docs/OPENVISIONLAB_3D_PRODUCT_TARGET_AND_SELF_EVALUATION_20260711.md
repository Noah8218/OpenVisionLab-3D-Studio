...
> 2026-07-22 architecture update: the canonical UI and recipe lifecycle is the generic `Inspection Recipe` Workbench using `ToolRecipeDocument`. Thickness, Warpage, Plane Flatness, Point Pair Dimensions, Gap/Flush, and Volume are Measure tool steps, not product modes. Schema `1.3` preserves exact `TransformedHeightField` GridRectangle/PointSet ownership, and Generic Ordered Recipe Executor v1 replays A3 then every following supported measurement step in authored order with direct-adapter hash parity. Volume fits `H = aU + bV + c` in a reference ROI and integrates signed residual times A3 cell area in a second ROI; values remain declared model-unit cubed and are not physical-volume claims. The focused Runner now passes `17/17`, and the current-build Shell passes real OS-pointer teaching, explicit Point Pair/Gap-Flush/Volume Preview and Publish, exact A3 binding, and schema `1.3` save/reopen. Reusable Plane Fit/Flatness, Point Pair, Gap/Flush, and Volume arithmetic is owned by committed Library-Noah `Lib.ThreeD 2.7.8`, while Studio retains identity/UI/recipe/result adaptation. See `OPENVISIONLAB_3D_VOLUME_TOOL_RECIPE_20260722.md` plus the earlier generic Measure documents. The historical owner-supplied Thickness/Warpage videos contribute only general synchronized step/overlay/properties/results and repeated-instance lessons; their visual identity, fixed layout, and fixed algorithm taxonomy are excluded. `Synthetic Affine Inspection Plate v1` remains the deterministic whole-chain `16/16` software golden. Automatic feature detection, the real four-landmark audit, and physical/metrology trust remain blocked by missing trusted evidence.
