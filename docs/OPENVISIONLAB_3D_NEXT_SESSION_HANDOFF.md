...
- NuGet package-health gate: `scripts\verify-nuget-package-health.py` parses JSON for vulnerable and deprecated direct/transitive packages, preserves both raw responses plus a summary, and returns failure for findings or incomplete/inconsistent query payloads. Each response project set must also match the eight projects declared by `OpenVisionLab.ThreeDStudio.slnx`. The four-case finding/schema/project-set self-test and current 8-project audit pass locally with zero findings on 2026-07-14; the exact restore/gate/build sequence also passes with zero build warnings/errors. CI runs the self-test and live audit as separate steps so either exit code independently gates the build, and `if: always()` preserves generated diagnostics. The new workflow steps have not yet been pushed or validated in Windows Actions.
