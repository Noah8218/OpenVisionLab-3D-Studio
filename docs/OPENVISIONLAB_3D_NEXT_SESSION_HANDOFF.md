...
The Inspection Recipe v1 baseline passes for five independent typed C3D slices, Durable Run Record v1.2 carries typed-step identity locally, and its schema v1.1 predecessor passes same-source repeatability across two authenticated Windows Cross-section runs. The repository runs on .NET 10, and the binary-only external WPF Host, Viewer/Shell screenshot quality, release identity, archive hash, and Viewer/Runner comparison pass locally and in Windows CI. Current source identifies as `0.1.1-dev`; `v0.1.0-rc.1` remains the immutable published Viewer-bundle prerelease. These remain tool-specific single-step recipe families, not a general multi-step executor, multi-piece evidence set, or metrology certification. Calibration and measured/nominal work remain blocked by missing metadata/data.
