...
- Public `3D/PublicSamples/glTF/ToyCar.glb` ad-hoc probe passes Viewer/Shell GLB load, pick, and two-point measurement with 66,951 vertices, 88,837 triangles, 66,951 UVs, embedded PNG texture evidence, and bounds-fit first-load camera distance `908.049`; it remains probe-only because it does not yet justify fixed-matrix runtime.
...
- Public `3D/PublicSamples/STL/3DBenchy.stl` ad-hoc probe passes Viewer/Shell STL load, pick, and two-point measurement with 677,118 vertices and 225,706 source triangles; Balanced render density displays 56,427 triangles with render stride 4, bounds-fit first-load camera distance is `170.030`, and the full probe completes in about 22 seconds after O(n) smoke measurement selection.
- Large imported-mesh samples remain probe-only unless they expose a loader, camera, picking, measurement, Shell, or contract gap not covered by the fixed matrix and the extra routine runtime is justified.
