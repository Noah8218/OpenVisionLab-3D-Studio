...
- Registration prototype decision: Open3D `DemoICPPointClouds` is a valid external alignment golden candidate with three distinct PCD fragments and transformation metadata, but it is not calibrated or nominal/actual evidence. A .NET 10 `PclNET 0.8.3` prototype loaded 198,835 points but exposed no ICP/registration API and produced a 61-file, 62,852,741-byte runtime, so no product dependency, PCD loader, or fixed sample was added. See `docs\OPENVISIONLAB_3D_REGISTRATION_ENGINE_PROTOTYPE_20260713.md`.
