...
The Inspection Recipe v1 baseline passes for five independent typed C3D slices, Durable Run Record v1.1 passes for one real Cross-section replay, the repository runs on .NET 10, and the binary-only external WPF Host, Shell screenshot quality, release identity, and Viewer/Runner comparison pass locally and in Windows CI. These remain tool-specific single-step recipe families, not a general multi-step executor or metrology certification. Calibration and measured/nominal work remain blocked by missing metadata/data; `0.1.0-rc.1` is pushed and CI-validated, while tag, package, and GitHub Release require separate approval.
...
4. Build one measured/nominal comparison slice when a distinct local sample pair is available; otherwise preserve the passed `0.1.0-rc.1` local/remote CI gates and await explicit tag/package/Release approval.
...
Obtain the C3D physical mapping/calibration contract and a genuinely distinct measured/nominal sample pair when available. Until then, preserve the pushed and CI-validated `0.1.0-rc.1` Viewer/BinaryHost/Shell-quality/release-identity/Viewer-Runner gates; do not tag, package, or publish the RC without explicit approval. Full CAD/GD&T, device/PLC/robot integration, enterprise data management, and AI tuning remain out of scope.
