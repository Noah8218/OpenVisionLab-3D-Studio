...
- The 2026-08-26 cross-process `ThreeD/HeightMap` smoke is complete: Machine
  Studio published the bundled Thickness Coupon C3D, a separate 3D consumer
  accepted it, materialized the raw buffer, and persisted a correlated
  `Completed/Pass` Result and Run Record. Preserve
  `INTEGRATION_3D_CROSS_PROCESS_HEIGHTMAP_SMOKE_20260826.md` and the D-backed
  run evidence named there. This is a console producer/consumer boundary, not
  WPF publish-button or clean-release evidence.
- The 2026-08-27 cross-modal projection slice is complete for the bounded
  software-coordinate adapter: the paired 2D Run Record retains actual
  `572x420` image geometry, the 3D consumer publishes 4 `2D->3D` raw-height
  points and 5 `3D->2D` ROI points, and the final Release smoke validates the
  paired profile, transaction/run identities, and separate producer/consumer
  processes. Preserve `INTEGRATION_3D_CROSS_MODAL_PROJECTION_20260827.md` and
  the D-backed cross-repository summary. This does not claim calibration,
  native viewer marker rendering, R0, quiet-host performance, release, or push.
- The follow-up recipe-owned HeightMap execution is complete: the consumer now
  verifies and loads the copied C3D Warpage recipe, derives ROI/acceptance from
  it, and rejects unit/frame mismatches before execution. The latest separate
  producer/consumer Pass evidence is recorded in
  `INTEGRATION_3D_RECIPE_OWNED_HEIGHTMAP_EXECUTION_20260826.md`. This remains
  a console adapter boundary and does not close the WPF execution or owner R0
  gates.
- The user-directed WPF follow-up now exposes an explicit `Accepted HeightMap`
  command in the Machine Studio exchange panel. The command is enabled only
  for an accepted `ThreeD/HeightMap` transaction without a Result and delegates
  to the recipe-owned adapter. The real Shell ViewModel verification passes
  `23/23`; current Release Shell build passes with `0` warnings and `0` errors;
  current Wide/Compact interaction-matrix evidence records the new disabled
  state, focus, hover, mouse-leave, CanExecute, and Tab behavior. A separate
  D-backed WPF smoke now also prepares an accepted external transaction and
  drives the visible `RunAcceptedHeightMap` button in Wide and Compact; both
  runs publish a correlated Result and disable rerun. Preserve
  `INTEGRATION_3D_WPF_HEIGHTMAP_EXECUTION_20260826.md` and its D-backed
  evidence. HeightMap execution now owns an asynchronous task and cancellation
  source, rejects duplicate execution, suppresses late UI refresh after
  shutdown, and gives `MainWindow` a two-second bounded close wait. The full UI
  gate remains open for deferred performance evidence, unrun DPI scales, and
  an in-flight desktop-close capture. The smoke-only identity injection does
  not weaken the production fail-closed identity path.
- Vendored Vision SDK package:
  `OpenVisionLab.Vision3D 3.0.1-dev.20260826.domain-mask.1`, built from
  committed `OpenVisionLab-Vision-SDK` source
  `db8b8a281dd028c62fabfc49febcde9b4d345d37`, SHA-256
  `D87570212D4C8913360CB01D20D9669720EDB6424B42C7FB790909EC8766D1CB`.
- `PL-0051 / G-11` is complete for the bounded software slice: an explicit
  source-bound C3D boolean mask is labeled by the vendored SDK and mapped to
  deterministic region cells, count, grid-index area, centroid, orientation,
  and cell-footprint bounds. Runner verification passes `10/10`; package
  verification passes `26/26`; the full Release build passes `0/0`. No
  polygon/circle rasterization, recipe/UI route, persisted region artifact,
  downstream consumer, calibrated area, version, release, commit, or push is
  implied. Preserve
  `OPENVISIONLAB_3D_CONNECTED_REGION_G11_CLOSURE_20260826.md` and
  `.proofline/issues/PL-0051.json`.
- `PL-0052 / G-12` is complete for the bounded presentation slice. An already
  evaluated G-11 `C3DConnectedRegionOutput` is projected into typed Workbench
  evidence with total/per-region count, grid-index area, center, orientation,
  bounds, and exact source-grid cells. Stable selection and `Show overlay`
  route the same output and region identity to the existing 3D Viewer and
  Height Image paths; source changes clear stale output, and presentation does
  not rerun detection or mutate recipe/source/ROI state. Release Runner
  connected-region verification passes `10/10`, Workbench `9/9`, Shell option
  verification `49/49`, Release tests pass `10/10`, and actual Release EXE
  smoke passes at Wide `1920x1040` and Compact `1280x760` on the dynamically
  selected leftmost monitor. Preserve
  ...
  Current runtime evidence is at `125%` DPI; other DPI scales, alternate
  themes, and owner R0 remain unverified or separately gated. Product version
  remains `0.1.1-dev`; no commit, push, tag, RC, or release occurred.
- `PL-0053 / G-13` is complete for the bounded Presence Check slice. One
  explicit source-bound `GridRectangle` is evaluated with inclusive finite
  coverage and raw-height limits; no finite mean fails closed as `Fail`.
  Typed `PresenceCheckResult` evidence preserves source/grid identity, unit,
  frame, exact feature region, counts, nullable mean, decision, reason, and
  deterministic hash through recipe validation, Workbench Preview/Publish,
  ordered Runner/Run Record, and JSON/HTML/CSV export. Runner passes `16/16`,
  Workbench `14/14`, Release build is `0/0`, Release tests are `10/10`, and
  actual Wide/Compact EXE screenshots pass at `1920x1040` and `1280x760` on
  the dynamically selected left monitor. Preserve
  `OPENVISIONLAB_3D_PRESENCE_CHECK_G13_CLOSURE_20260826.md`,
  `.proofline/issues/PL-0053.json`, and the D-backed `G-13` evidence root.
  Current runtime evidence is at `125%` DPI; alternate themes, other DPI
  scales, the full UI performance baseline, owner R0, maximum-C3D
  qualification, calibration, and release qualification remain unverified or
  separately gated. Product version remains `0.1.1-dev`; no commit, push,
  tag, RC, or release occurred.
...
The next executable software priority after the bounded G-13 closure is G-14:
add Fill Height per region against a reference surface using a known fill-level
synthetic fixture. Recommended model: `gpt-5.6-sol`. Reasoning effort: high.
This priority remains separate from the owner R0, maximum-C3D, physical
metrology, and release prerequisites.
...
The bounded recipe-owned HeightMap follow-up, WPF visible-click follow-up, and
bounded asynchronous cancellation/shutdown follow-up are implemented. The
next software priority is representative maximum-C3D memory/load-time
qualification, but it is blocked until a representative maximum input and
accepted memory/load-time limits are supplied. Recommended model: none until
the prerequisite exists; reasoning effort: none.

The WPF slice remains `Incomplete` as a full UI closure because performance
evidence was explicitly deferred and only the available 125% DPI was run.
Re-read the master backlog and current handoff when an external prerequisite
changes.
...
```text
Status: Incomplete
Scope: Historical pre-async slice: user-directed WPF Accepted HeightMap execution for an accepted ThreeD/HeightMap Handoff, including recipe-owned delegation, Result refresh, explicit enablement, Wide/Compact interaction evidence, and visible accepted-transaction button execution
Acceptance criteria: accepted ThreeD/HeightMap without Result enables the command -> pass; pre-acknowledgement and post-Result states disable it -> pass; recipe-owned execution publishes the correlated Result/Run Record -> pass; current Release Shell build -> pass; current Wide/Compact interaction matrix and screenshot quality -> pass; visible accepted-transaction click publishes Result and disables rerun -> pass; performance smoke -> intentionally deferred
Verification: WPF Shell ViewModel verification 20/20; Reporting tests 7/7; Shell smoke options 48/48; current Release Shell build 0/0; final Wide/Compact click-smoke scripts pass; monitor intersection and 125% DPI recorded; PowerShell parse and git diff --check pass
Evidence: docs/INTEGRATION_3D_WPF_HEIGHTMAP_EXECUTION_20260826.md; D:/OpenVisionLab-TestData/OpenVisionLab-3D-Studio/wpf-heightmap-click-smoke/run-20260826-184554-8ff7bf216ea046039e5d1d179f4f02cd/; D:/OpenVisionLab-TestData/OpenVisionLab-3D-Studio/wpf-heightmap-click-smoke/run-20260826-184608-46b6df4bd57547f6a4209e3b153ca4a6/; D:/OpenVisionLab-TestData/OpenVisionLab-3D-Studio/wpf-heightmap-click-smoke/integration-view-model-after-click.txt; D:/OpenVisionLab-TestData/OpenVisionLab-3D-Studio/wpf-heightmap-click-smoke/shell-options-final.txt
Boundary / next dependency at that time: performance baseline and 100/150/175/200% DPI remained unverified by explicit scope; bounded asynchronous cancellation/shutdown was the next software priority; owner R0, release gates, and maximum-C3D qualification remained external
```

```text
Status: Incomplete
Scope: User-directed bounded asynchronous HeightMap execution and MainWindow close coordination for the accepted ThreeD/HeightMap exchange path
Acceptance criteria: ViewModel owns one in-flight task and cancellation source -> pass; duplicate command is rejected while running -> pass; ShutdownAsync requests cancellation and returns at the supplied bound -> pass; late completion does not refresh UI state after shutdown -> pass; MainWindow performs a two-second bounded close wait and repeats close deterministically -> source/build pass; in-flight real-desktop close capture -> not run; performance smoke -> intentionally deferred
Verification: WPF Shell ViewModel verification 23/23; Reporting tests 8/8 without warnings; current Release Shell build 0/0; final Wide/Compact visible-click smoke passes with asynchronous completion and bounded close re-entry; Shell smoke options 48/48 via the built DLL; PowerShell parse and git diff --check pass
Evidence: docs/INTEGRATION_3D_WPF_HEIGHTMAP_EXECUTION_20260826.md; D:/OpenVisionLab-TestData/OpenVisionLab-3D-Studio/heightmap-async-viewmodel-final-20260826.txt; D:/OpenVisionLab-TestData/OpenVisionLab-3D-Studio/shell-options-async-shutdown-dll-20260826.txt; D:/OpenVisionLab-TestData/OpenVisionLab-3D-Studio/wpf-heightmap-click-smoke/run-20260826-192019-4b7e0dca6238418a908ec9e36d9b19f2/; D:/OpenVisionLab-TestData/OpenVisionLab-3D-Studio/wpf-heightmap-click-smoke/run-20260826-192035-4a4c754ff78d4803995731a110f3e598/
Boundary / next dependency: cancellation is cooperative around synchronous C3D parse/hash/SDK calls and can detach after timeout; full WPF closure still requires the user-deferred performance baseline, 100/150/175/200% DPI, and an actual in-flight desktop-close capture; no PC restart, commit, push, version, package, release, or deployment occurred
```
