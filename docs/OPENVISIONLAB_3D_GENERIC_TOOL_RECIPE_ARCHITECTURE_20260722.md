...
- Artifact-owned selection and ordered Runner verification: schema/route, legacy single-sequence compatibility, five-measurement authored order, direct-adapter hash parity, continued evidence after tolerance Fail, ROI/PointSet save/reopen, and wrong owner/hash/grid/order/tool/output rejection pass `16/16`.
