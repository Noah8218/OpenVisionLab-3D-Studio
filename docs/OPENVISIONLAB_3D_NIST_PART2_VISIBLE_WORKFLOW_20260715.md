...
This closes the Viewer Reliability Phase 2 second-pair checklist item. It does not close Phase 2 as a whole: the controlled difficult-geometry matrix and registration-acceptance checklist items remain open. It is not arbitrary-geometry, arbitrary-alignment, calibration, uncertainty, or metrology evidence.
...
Phase 2 remains open. The next evidence task is to audit the existing controlled cases against the reliability checklist for duplicate vertices, non-finite normals, open surfaces, edge/vertex nearest hits, sparse/dense query sampling, and no-correspondence behavior. Add only the missing cases. Registration acceptance must then record correspondence count and fitness before RMSE and reject zero-correspondence false success in the accepted product execution path.
