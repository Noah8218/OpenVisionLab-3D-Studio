...
> 2026-07-24 현재 동작: Validation 실패 분석 체크포인트에서
> Workbench:Evidence 비율을 아래 기록의 `1.2:1`에서 `0.82:1`로 확장했다.
> 고정 `1920 x 1040` 실제 EXE에서 원본/실패 샘플 Viewer 두 개가 완전히
> 보이는 것이 현재 증거다. 자세한 내용은
> `OPENVISIONLAB_3D_VALIDATION_FAILURE_ANALYSIS_UX_20260724.md`를 따른다.
