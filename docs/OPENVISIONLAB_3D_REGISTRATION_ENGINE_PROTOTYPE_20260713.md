...
The official Open3D `0.19.0` Windows C++ package and a same-tag source build are technically viable separate-process registration candidates, not approved product dependencies. The source configuration now passes both a recovered build and an independent clean single-shot Release build/install and reduces the adjacent probe runtime from `141,536,768` to `58,520,064` bytes. The clean-build probe matches the official binary exactly in all 33 robustness runs and all three current `0 -> 1` DemoICP runs. Both runtimes reject `1 -> 2` because `cloud_bin_2.pcd` contains 771 non-finite normals; the older successful `1 -> 2` metrics predate this guard. A schema-valid 52-component CycloneDX candidate records 27 direct and 25 support/transitive components. Assimp's fixed-build compiled closure, fixed oneMKL wheel/payload provenance, and VTK source/recipe/payload/transitive closure now pass. Assimp vendored snapshot/modification provenance, BoringSSL binary/toolchain reproducibility, and VTK's VS2019-workflow versus `_MSC_VER=1900` binary conflict keep the full manifest gate open. The distribution audit still blocks publication because final notices, Microsoft runtime handling, clean-host evidence, product integration impact, and owner/legal approval have not passed; see `docs/OPENVISIONLAB_3D_OPEN3D_DISTRIBUTION_AUDIT_20260713.md`. Viewer/Runner transform parity also remains open.

**2026-07-16 correction:** the earlier `VS2019` versus `_MSC_VER=1900` VTK conflict wording is withdrawn. The legacy package's `vtkConfigureDeprecated.h` records MSVC `14.29.30133` under Visual Studio 2019, and current VS2022 `yvals.h` deliberately emits the same `1900` marker. A no-patch controlled Release rebuild now passes Release-path, target-contract, directive, and direct link/run checks. A follow-up same-source `USE_SYSTEM_VTK=ON` Open3D build preserves the independent clean build's 873 install paths, 29 dependency entries, 16,000 export entries, DemoICP behavior, controlled `1 -> 2` rejection, and 33-run robustness output. This is local candidate-runtime evidence only; it is not a Debug rebuild, historical byte-identical reconstruction, Open3D product integration approval, or distribution approval. See `docs/OPENVISIONLAB_3D_VTK_CONTROLLED_REBUILD_20260716.md` and `docs/OPENVISIONLAB_3D_OPEN3D_VTK_CANDIDATE_RUNTIME_20260716.md`.

**2026-07-16 distribution-evidence correction:** the preceding historical summary is superseded in five narrow ways. First, the CMake-selected official Assimp `v5.4.2` ZIP now matches all `2,940/2,940` clean-build source paths, lengths, and SHA-256 values with empty update/patch commands; original Poly2Tri Mercurial `5de962...` to candidate Git `99927...` archive exports match in all `35/35` raw-byte-identical files and current Assimp Poly2Tri history covers all `14` direct-delta paths with `28` official history entries and `1,407/1,328` net delta evidence; fixed Clipper `6.4.2` release/import/current-tag provenance also passes at `7/6` archive-to-import and `4/4` import-to-current with current build blob `c0a8565bb98568dcca4a5350ca52fa08152bea51`; fixed stb_image upstream commit `0bc88af...` to current build source passes with shared blob `a632d543510ebf4410f124369b07a303e1d096d6`, with no upstream release/tag claim. Other vendored-component upstream revision/modification provenance still remains open. Second, the fixed Open3D candidate now has a Windows Sandbox technical clean-host prerequisite pass, not merely local preflight: the guest fails before the reviewed VC/OpenMP prerequisite and passes the fixed `0 -> 1` probe afterward. REDIST terms, notices, owner/legal approval, product impact, real result mapping, and Viewer/Runner parity remain open. Third, the VTK marker conflict is withdrawn as stated above. The candidate therefore remains excluded from product dependencies and release bundles. See `docs/OPENVISIONLAB_3D_ASSIMP_SOURCE_SNAPSHOT_IDENTITY_20260716.md`, `docs/OPENVISIONLAB_3D_ASSIMP_POLY2TRI_LINEAGE_20260716.md`, `docs/OPENVISIONLAB_3D_ASSIMP_POLY2TRI_DELTA_ATTRIBUTION_20260716.md`, `docs/OPENVISIONLAB_3D_ASSIMP_CLIPPER_PROVENANCE_20260716.md`, `docs/OPENVISIONLAB_3D_ASSIMP_STB_PROVENANCE_20260716.md`, `docs/OPENVISIONLAB_3D_OPEN3D_CLEAN_HOST_EXECUTION_PROTOCOL_20260716.md`, and `docs/OPENVISIONLAB_3D_OPEN3D_DISTRIBUTION_AUDIT_20260713.md`.

No downloaded candidate file or prototype dependency is committed to the fixed sample matrix.

**2026-07-16 kuba--zip correction:** the three Assimp compiler inputs now pass from public `kuba--/zip v0.3.1` through a bounded, CRLF-normalized Assimp delta to the exact `v5.4.2` clean-build input. The vendored CMake `0.3.0` metadata is not treated as source identity. This narrows one supply-chain gap only; independent miniz provenance, other components, notices, distribution, product adoption, and Viewer/Runner parity remain blocked. See `docs/OPENVISIONLAB_3D_ASSIMP_KUBAZIP_PROVENANCE_20260716.md`.

**2026-07-16 pugixml correction:** the three effective Assimp header-only inputs now pass from public `zeux/pugixml v1.13` commit `a0e064336317c9347a91224112af9933598714e9` through import `62cefd5b275628ff97a77d0cd9220e1c35794a3f` to the exact `v5.4.2` clean-build source. `pugixml.cpp` and `pugixml.hpp` are source-identical at import after CRLF normalization, while `pugiconfig.hpp` retains the explicit header-only configuration. The standalone CMake `VERSION 1.9` value is metadata, not the source identity. This narrows one supply-chain gap only; remaining components, notices, distribution, product adoption, and Viewer/Runner parity remain blocked. See `docs/OPENVISIONLAB_3D_ASSIMP_PUGIXML_PROVENANCE_20260716.md`.

**2026-07-16 UTF8-CPP correction:** the four compiler-read headers now pass from public `nemtrif/utfcpp v3.2.3` commit `79835a5fa57271f07a90ed36123e30ae9741178e`, through Assimp update `ce59d49dd9ce93ccf8585f78c70e58cb0e5d4961`, to the exact `v5.4.2` clean-build source. All four fixed blobs match and there is no post-update path history. CMake, the `utf8.h -> checked/unchecked -> core` path, and the recorded four-file compiler-read closure pass. This narrows one four-header supply-chain gap only; optional headers, remaining components, notices, distribution, product adoption, and Viewer/Runner parity remain blocked. See `docs/OPENVISIONLAB_3D_ASSIMP_UTF8CPP_PROVENANCE_20260716.md`.

**2026-07-16 MiniZip correction:** the four compiler-read MiniZip files now pass from public `madler/zlib v1.3.1` commit `51b7f2abdade71cd9bb0e7a373ef2610ec6f9daf`, through Assimp update `64d88276ef7117c09165e468dbb9acd999e324ac`, to the exact `v5.4.2` clean-build source. All four fixed blobs match and there is no post-update path history. CMake lists `crypt.h`, but source-defined `NOUNCRYPT` prevents its conditional inclusion in this build, so it is outside the recorded four-file closure. This narrows one bounded zlib-contrib supply-chain gap only; complete MiniZip/Info-ZIP/`crypt.h` provenance, other components, notices, distribution, product adoption, and Viewer/Runner parity remain blocked. See `docs/OPENVISIONLAB_3D_ASSIMP_MINIZIP_PROVENANCE_20260716.md`.

**2026-07-16 miniz correction:** the one compiler-read miniz header now passes from public `kuba--/zip v0.3.1`, through the fixed Assimp PR changes, merge, post-merge change, and the exact `v5.4.2` clean-build input. PR head and merge have exact content identity but no claimed Git ancestry. This narrows one bounded Kuba-to-Assimp supply-chain gap only; independent original `richgel999/miniz` identity/history, other components, notices, distribution, product adoption, and Viewer/Runner parity remain blocked. See `docs/OPENVISIONLAB_3D_ASSIMP_MINIZ_PROVENANCE_20260716.md`.

**2026-07-16 OpenDDL Parser correction:** the `13` compiler-read inputs now pass from public `kimkulling/openddl-parser v0.5.1`, through the exact Assimp baseline, the two fixed source changes, and the exact `v5.4.2` clean-build inputs. The static `0.4.0` source string is upstream metadata rather than tag identity. This narrows one bounded source/delta supply-chain gap only; other components, notices, distribution, product adoption, and Viewer/Runner parity remain blocked. See `docs/OPENVISIONLAB_3D_ASSIMP_OPENDDL_PROVENANCE_20260716.md`.

**2026-07-16 zlib correction:** the `25` compiler-read zlib core inputs now pass from public `madler/zlib v1.2.13`, through the exact Assimp update, and the exact `v5.4.2` clean-build inputs. Every source blob matches and the checked source-path history is empty after import. The generated `zconf.h` boundary remains explicit and outside the upstream source-identity subset. This narrows one bounded source/delta supply-chain gap only; other components, notices, distribution, product adoption, and Viewer/Runner parity remain blocked. See `docs/OPENVISIONLAB_3D_ASSIMP_ZLIB_PROVENANCE_20260716.md`.

**2026-07-16 RapidJSON correction:** the `29` compiler-read headers now pass from public post-`v1.1.0` RapidJSON snapshot `676d99...`, through the exact Assimp update, and the exact `v5.4.2` clean-build inputs. The `v1.1.0` tag remains an ancestor, not source identity. Every header blob matches and the checked source-path history is empty after import. This narrows one bounded source/delta supply-chain gap only; other components, notices, distribution, product adoption, and Viewer/Runner parity remain blocked. See `docs/OPENVISIONLAB_3D_ASSIMP_RAPIDJSON_PROVENANCE_20260716.md`.

**2026-07-16 Open3DGC correction:** the `29` compiler-read files now pass from public `KhronosGroup/glTF` `mesh-compression-open3dgc` snapshot `7b61d5e...`, through Assimp import `054820e6...`, to the exact `v5.4.2` clean-build inputs. The fixed current delta is exactly `16` paths. The source notices are `MIT AND BSD-2-Clause`, not MIT-only. This narrows one public carrier-snapshot/import/delta supply-chain gap only; historical AMD remote availability, final notices, distribution, product adoption, and Viewer/Runner parity remain blocked. See `docs/OPENVISIONLAB_3D_ASSIMP_OPEN3DGC_PROVENANCE_20260716.md`.
...
The distant-initial case is the critical controlled-failure finding. Open3D reports inlier RMSE `0` when no correspondences exist, so RMSE alone can misclassify a complete registration failure as perfect. The runtime-neutral product acceptance contract now requires a minimum correspondence count and fitness before evaluating RMSE, reports `NoCorrespondences` explicitly, and keeps transform plausibility and scenario-specific residual limits separate. Real measured/nominal acceptance thresholds remain blocked until representative paired data and physical units are available.
...
| Robustness characterization and acceptance policy | Policy pass; runtime blocked | The 11-case prototype remains only partially accepted; the runtime-neutral product rule passes `20/20` locally and in Windows CI and rejects zero-correspondence RMSE `0`, but no approved runtime feeds it yet |
| Runtime, license, and host impact explicit | Partial; blocked for distribution | Same-tag recovered and independent clean builds, plus the controlled-VTK Open3D runtime check, preserve the 873-path install contract, 29 dependencies, 16,000 exports, DemoICP, and 33-run clean-build parity. The 52-component direct-evidence CycloneDX candidate, Assimp compiled closure, fixed Poly2Tri/Clipper/Open3DGC/stb_image/kuba--zip/pugixml/UTF8-CPP/MiniZip/miniz/OpenDDL Parser/zlib core/RapidJSON provenance, fixed oneMKL payload provenance, VTK source/recipe/payload/closure, and technical VC/OpenMP clean-host execution pass; remaining independent miniz, Assimp/BoringSSL/VTK binary provenance, final notices, REDIST/legal treatment, product integration impact, and owner/legal approval remain open |
