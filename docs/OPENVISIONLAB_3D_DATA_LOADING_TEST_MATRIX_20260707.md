...
| `3D/PublicSamples/glTF/SimpleInstancing.glb` | GLB mesh with static `EXT_mesh_gpu_instancing` | Inventory, import, render, scene/node traversal, static instance expansion, mesh bounds, triangle/vertex contract, triangle-surface pick, two-point distance/model-Y height comparison, Shell context. | `artifacts/matrix_glb_simple_instancing_after.png`, `artifacts/matrix_glb_simple_instancing_pick_after.png`, `artifacts/matrix_glb_simple_instancing_measure_after.png`, `artifacts/matrix_shell_glb_simple_instancing_measure_after.png`, `artifacts/matrix_glb_simple_instancing_measure_after.txt` | Proves one static glTF extension path only; does not prove animation, skinning, morph targets, material extension, or general extension support. |
...
| `3D/PublicSamples/glTF/ToyCar.glb` | GLB realistic larger textured mesh | Inventory, import, render, mesh bounds, vertex/triangle contract, UV/texture contract, fit camera behavior, triangle-surface pick, triangle-index/normal metadata, two-point measurement, Shell context. | `artifacts/matrix_glb_toycar_after.png`, `artifacts/matrix_glb_toycar_pick_after.png`, `artifacts/matrix_glb_toycar_measure_after.png`, `artifacts/matrix_shell_glb_toycar_measure_after.png`, `artifacts/matrix_glb_toycar_measure_after.txt` | Higher-vertex real GLB sample for performance/coverage before large-stable-mesh expansion. |
...
| `3D/PublicSamples/STL/3DBenchy.stl` | STL triangle mesh with high triangle count | Inventory, import, render, mesh bounds, large source triangle count, render stride, surface pick, normal overlay, two-point measurement, Shell context. | `artifacts/matrix_stl_3dbenchy_after.png`, `artifacts/matrix_stl_3dbenchy_pick_after.png`, `artifacts/matrix_stl_3dbenchy_measure_after.png`, `artifacts/matrix_shell_stl_3dbenchy_measure_after.png`, `artifacts/matrix_stl_3dbenchy_measure_after.txt` | High-triangle STL sample for sampled render and distance math stability at imported-mesh scale. |
...
| --- | --- | --- | --- |
| `3D/PublicSamples/glTF/ToyCar.glb` | Promoted to fixed matrix | Fixed matrix now includes ToyCar load, pick, and measurement with full contract evidence (`matrix_glb_toycar_after.txt`, `matrix_glb_toycar_pick_after.txt`, `matrix_glb_toycar_measure_after.txt`). Legacy probe artifacts remain for historical comparison (`artifacts/probe_toycar_after_continue`). | Previous ad-hoc-only category retired into routine matrix coverage as of 2026-07-09. |
| `3D/PublicSamples/STL/3DBenchy.stl` | Promoted to fixed matrix | Fixed matrix now includes 3DBenchy load, pick, and measurement with full contract evidence (`matrix_stl_3dbenchy_after.txt`, `matrix_stl_3dbenchy_pick_after.txt`, `matrix_stl_3dbenchy_measure_after.txt`). Legacy probe artifacts remain for historical comparison (`artifacts/probe_3dbenchy_after_continue`). | Previous ad-hoc-only category retired into routine matrix coverage as of 2026-07-09. |

## Fixed Matrix Promotion Policy

Decision as of 2026-07-09: promote `ToyCar.glb` and `3DBenchy.stl` into the fixed matrix with load/pick/measurement and Shell measurement coverage.

Promote an ad hoc sample into the fixed matrix only when all of these are true:

1. It exposes a loader, camera, picking, measurement, Shell, contract, or failure-handling gap that the current fixed matrix does not cover.
2. The sample source, license, size, and SHA256 are documented in `3D/PublicSamples/README.md`.
3. Current-source evidence includes Viewer load, relevant pick/measurement, Shell context, and contract text.
4. The extra runtime is acceptable for routine local and CI validation.

Keep a sample probe-only when it is mainly a stress/performance check, when it duplicates an already-covered behavior, or when it is useful before risky mesh/point-cloud rendering changes but too expensive for every matrix run.

Use these current large-probe refresh commands when touching imported-mesh camera, picking, measurement, HUD, render density, or STL/GLB loading:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\probe-3d-sample.ps1 -SamplePath 3D\PublicSamples\glTF\ToyCar.glb -ArtifactDir artifacts\probe_toycar_after -SkipBuild
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\probe-3d-sample.ps1 -SamplePath 3D\PublicSamples\STL\3DBenchy.stl -ArtifactDir artifacts\probe_3dbenchy_after -SkipBuild
```
...
The script writes `artifacts/matrix_smoke_summary_after.txt` by default and treats expected invalid-sample loader failures as passing checks when the process exit code is `1`. If you need a stable per-run folder, pass `-ArtifactDir`, for example `-ArtifactDir artifacts\matrix_after_continue_full` (as used in the latest run).
...
dotnet run --project src\OpenVisionLab.ThreeDStudio\OpenVisionLab.ThreeDStudio.csproj -c Debug --no-build -- --smoke-screenshot artifacts\matrix_glb_simple_instancing_after.png --smoke-glb 3D\PublicSamples\glTF\SimpleInstancing.glb --smoke-contracts artifacts\matrix_glb_simple_instancing_after.txt
dotnet run --project src\OpenVisionLab.ThreeDStudio\OpenVisionLab.ThreeDStudio.csproj -c Debug --no-build -- --smoke-screenshot artifacts\matrix_glb_simple_instancing_pick_after.png --smoke-glb 3D\PublicSamples\glTF\SimpleInstancing.glb --smoke-pick glb --smoke-contracts artifacts\matrix_glb_simple_instancing_pick_after.txt
dotnet run --project src\OpenVisionLab.ThreeDStudio\OpenVisionLab.ThreeDStudio.csproj -c Debug --no-build -- --smoke-screenshot artifacts\matrix_glb_simple_instancing_measure_after.png --smoke-glb 3D\PublicSamples\glTF\SimpleInstancing.glb --smoke-measure glb-two-point --smoke-contracts artifacts\matrix_glb_simple_instancing_measure_after.txt
...
dotnet run --project src\OpenVisionLab.ThreeDStudio\OpenVisionLab.ThreeDStudio.csproj -c Debug --no-build -- --smoke-screenshot artifacts\matrix_glb_toycar_after.png --smoke-glb 3D\PublicSamples\glTF\ToyCar.glb --smoke-contracts artifacts\matrix_glb_toycar_after.txt
dotnet run --project src\OpenVisionLab.ThreeDStudio\OpenVisionLab.ThreeDStudio.csproj -c Debug --no-build -- --smoke-screenshot artifacts\matrix_glb_toycar_pick_after.png --smoke-glb 3D\PublicSamples\glTF\ToyCar.glb --smoke-pick glb --smoke-contracts artifacts\matrix_glb_toycar_pick_after.txt
dotnet run --project src\OpenVisionLab.ThreeDStudio\OpenVisionLab.ThreeDStudio.csproj -c Debug --no-build -- --smoke-screenshot artifacts\matrix_glb_toycar_measure_after.png --smoke-glb 3D\PublicSamples\glTF\ToyCar.glb --smoke-measure glb-two-point --smoke-contracts artifacts\matrix_glb_toycar_measure_after.txt
...
dotnet run --project src\OpenVisionLab.ThreeD.Shell\OpenVisionLab.ThreeD.Shell.csproj -c Debug --no-build -- --shell-smoke-screenshot artifacts\matrix_shell_glb_simple_instancing_measure_after.png --smoke-glb 3D\PublicSamples\glTF\SimpleInstancing.glb --smoke-measure glb-two-point
dotnet run --project src\OpenVisionLab.ThreeD.Shell\OpenVisionLab.ThreeD.Shell.csproj -c Debug --no-build -- --shell-smoke-screenshot artifacts\matrix_shell_glb_toycar_measure_after.png --smoke-glb 3D\PublicSamples\glTF\ToyCar.glb --smoke-measure glb-two-point
...
dotnet run --project src\OpenVisionLab.ThreeD.Shell\OpenVisionLab.ThreeD.Shell.csproj -c Debug --no-build -- --shell-smoke-screenshot artifacts\matrix_shell_stl_3dbenchy_measure_after.png --smoke-stl 3D\PublicSamples\STL\3DBenchy.stl --smoke-measure mesh-two-point
...
| `artifacts/matrix_glb_simple_instancing_after.txt` | `GLB|loaded=True`, `SimpleInstancing.glb`, `vertices=3000`, `triangles=1500`, and expanded instance bounds from roughly `(-1.667, -1.667, -1.667)` to `(12.732, 12.732, 12.732)`. |
| `artifacts/matrix_glb_simple_instancing_pick_after.txt` | `GLBPick|selected=True|kind=mesh surface` with triangle index and normal on an expanded instance. |
| `artifacts/matrix_glb_simple_instancing_measure_after.txt` | `SelectionMode|value=Two Point Measure`, `MeasurementOverlay|visible=True`, and `TwoPoint|visible=True` on the expanded instanced geometry. |
| `artifacts/matrix_glb_toycar_after.txt` | `GLB|loaded=True`, `ToyCar.glb`, `vertices=77429`, `triangles=108936`, `hasTexture=True`, and bounds `(-0.035, -0.016, -0.037)` to `(0.038, 0.013, 0.037)`. |
| `artifacts/matrix_glb_toycar_pick_after.txt` | `GLBPick|selected=True|kind=mesh surface`, and `triangleIndex=` / `normal=` values. |
| `artifacts/matrix_glb_toycar_measure_after.txt` | `SelectionMode|value=Two Point Measure`, `MeasurementOverlay|visible=True`, `TwoPoint|visible=True`, `distance=`, `dX=`, `dY=`, `dZ=`, and model-Y height delta. |
...
| `artifacts/matrix_stl_3dbenchy_after.txt` | `STL|loaded=True`, `vertices=677118`, `triangles=225706`, and bounds `(-29.176, -15.502, 0.000)` to `(30.825, 15.502, 48.000)`. |
| `artifacts/matrix_stl_3dbenchy_pick_after.txt` | `STLPick|selected=True|kind=mesh surface`, `triangleIndex=`, `normal=`, and `STLSurfaceOverlay|visible=True`. |
| `artifacts/matrix_stl_3dbenchy_measure_after.txt` | `SelectionMode|value=Two Point Measure`, `MeasurementOverlay|visible=True`, `LAZAcceptance|visible=False`, and `TwoPoint|visible=True` with distance and model-Y height delta. |
...
| `artifacts/matrix_after_continue_full/matrix_smoke_summary_after.txt` | PASS lines for build, C3D/GLB/STL/LAS/LAZ positive samples, expected missing/corrupt failures, and contract text checks (latest full run). |
...
Next implementation should run the next real GLB/STL/LAS/LAZ dataset through `scripts\probe-3d-sample.ps1` first, then add it to this fixed matrix only if it satisfies the promotion policy above. Unsupported OBJ, PLY, PCD, E57, STEP, or IGES files should be recorded as format-expansion candidates instead of forcing them into the current supported-format matrix.
