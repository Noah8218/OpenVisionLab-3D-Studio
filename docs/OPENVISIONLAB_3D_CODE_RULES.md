...
- 루트 `README.md`는 영어로 작성한다.
- 공개 제품 설명은 사용자가 수행할 수 있는 검사 흐름과 기능을 우선한다.
  카메라, 조명, PLC, 산업용 I/O, 로봇, 클라우드처럼 지원하지 않는 플랫폼
  목록을 README 또는 공개 제품 문서의 제품 소개 문구로 사용하지 않는다.
- 포함 예제는 `Thickness Coupon`처럼 검사 목적을 기준으로 이름을 붙인다.
  데이터 생성 방식, 재현 해시, 데이터 안전성 근거는 개발 증거 문서가
  소유하며 README 첫 화면이나 사용자용 예제 이름에 노출하지 않는다.
- README GIF와 공개 화면은 애플리케이션 창만 포함해야 한다. 바탕화면,
  작업표시줄, 다른 프로그램, 사용자 계정, 알림, 로컬 경로가 한 프레임이라도
  노출되면 배포하지 않는다.
- 검사 GIF의 ROI는 실제로 설명하는 검사 대상과 일치해야 한다. Thickness는
  같은 부품의 기준면과 측정면을 한 쌍으로 보여야 한다.
- 루트 `LICENSE`와 `NOTICE`를 유지하고 README의 License 섹션에서 실제
  라이선스로 연결한다.
