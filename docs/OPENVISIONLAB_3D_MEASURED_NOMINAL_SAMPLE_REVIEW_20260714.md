...
The second-pair, supplied-transform, and local difficult-geometry controlled-outcome gates now pass, but Phase 2 itself does not. The synthetic matrix passes mesh deviation `23/23` and nominal/actual execution `29/29`; see `docs/OPENVISIONLAB_3D_PHASE2_DIFFICULT_GEOMETRY_GOLDENS_20260715.md`. Its mandatory fail-closed workflow step passes locally, while the first GitHub Windows execution and registration acceptance remain open. Registration recovery stays separate from Stanford's supplied-transform application and is blocked from product integration until its runtime/distribution prerequisites are resolved.
